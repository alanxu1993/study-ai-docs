...
![MCP 是连接外部世界的 USB 接口](/images/claude-code/mcp/01-usb-hub.png)
...
![三种接入方式：HTTP 推荐、SSE 已弃用、stdio 本地](/images/claude-code/mcp/02-transport-roads.png)
...
* **输出令牌限制**：MCP 输出超 10000 令牌触发警告，可通过环境变量扩容：
  `bash export MAX_MCP_OUTPUT_TOKENS=50000 # 临时扩容（当前会话有效） echo "export MAX_MCP_OUTPUT_TOKENS=50000" >> ~/.bashrc # 永久扩容（Linux/macOS）`
