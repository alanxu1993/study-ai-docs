...
![插件把 Skills/Hooks/Agents/MCP 打包成可安装单元](/images/claude-code/plugins/01-plugin-pack.png)
...
![CLI 管理插件全生命周期：install / enable / update](/images/claude-code/plugins/02-cli-install.png)
...
| Go | gopls | go install [golang.org/x/tools/gopls@latest](https://golang.org/x/tools/gopls%40latest) |
...
| 钩子未触发 | 1. 事件名称大小写错误；2. 脚本无执行权限；3. 路径错误 | 1. 核对事件名称（如PostToolUse）；2. 执行chmod +x 脚本名；3. 使用${CLAUDE\_PLUGIN\_ROOT}配置路径 |
| MCP服务器启动失败 | 1. 路径未用${CLAUDE\_PLUGIN\_ROOT}；2. 服务器未安装；3. 参数错误 | 1. 统一使用环境变量配置路径；2. 安装对应服务器；3. 核对命令和参数 |
